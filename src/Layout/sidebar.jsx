...
// import { Home,BookOpen, Clipboard, Settings, BookAIcon, InfoIcon, Tag, CreditCard } from "lucide-react";// Added a fallback icon for logo if needed
  ...
  return (
    <>
      ...
      */}
      <motion.aside
        className={`
          fixed z-50 bg-white dark:bg-slate-900 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] md:shadow-2xl flex transition-colors duration-300
          
          /* Mobile Styles */
          bottom-0 left-0 w-full h-[70px] flex-row items-center justify-between px-4 border-t border-gray-200 dark:border-slate-800
      ...
      >
        ...
        <nav className="flex-1 flex flex-row md:flex-col md:mt-4 gap-1 md:gap-2 w-full justify-between md:justify-start md:px-2">
            ...
            return (
              <motion.button
                ...
                className={`
                  ...
                  ${
                    active
                      ? "text-amber-600 md:bg-gradient-to-r md:from-orange-400 md:to-amber-400 md:text-white md:shadow-md dark:text-amber-400 md:dark:text-white"
                      : "text-gray-500 dark:text-gray-400 hover:bg-amber-50 md:hover:bg-amber-100 dark:md:hover:bg-slate-800 cursor-pointer"
                  }
                `}
                whileTap={{ scale: 0.9 }}
              >
          ...
          <button
            onClick={() => navigate("/settings")}
            className={`
              flex md:hidden flex-col items-center justify-center flex-1 p-2 text-gray-500 dark:text-gray-400
            `}
          >
             <Settings size={20} />
             <span className="text-[10px] font-medium">Set</span>
        ...
        {/* ---------------- SETTINGS (Desktop Only Position) ---------------- */}
        <div className="hidden md:block mt-auto p-4 border-t border-amber-100 dark:border-slate-800">
          <button
            onClick={() => navigate("/settings")}
            className="flex items-center gap-3 px-3 py-2 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-amber-100 dark:hover:bg-slate-800 w-full transition-all duration-200 cursor-pointer"
          >
            <Settings size={20} />
            <AnimatePresence>
              {hovered && (
      ...
      <div
        className={`
          flex-1 flex flex-col h-screen overflow-y-auto transition-all duration-300 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100
          /* Mobile: Remove left margin, add bottom padding for nav bar */
          ml-0 pb-24
