...
import LogImg from "../Components/leftSideImg";
import axios from "axios";
import { Sun, Moon } from "lucide-react"; // Added for toggle
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center font-poppins overflow-hidden bg-gradient-to-br from-amber-950 via-amber-800 to-orange-700 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 transition-colors duration-500">
      ...
      >
        ...
        >
          <h1 className="text-4xl font-bold mb-2 bg-gradient-to-r from-amber-300 to-orange-400 dark:from-amber-400 dark:to-amber-200 bg-clip-text text-transparent">
            Reset Password 🔐
          </h1>
          <p className="text-gray-300 dark:text-slate-400 mb-8">
            Enter the OTP sent to your email and set your new password
          </p>

          {error && <p className="text-red-400 mb-3 text-sm">{error}</p>}
          {message && <p className="text-amber-300 dark:text-amber-500 mb-3 text-sm">{message}</p>}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col">
            ...
            <div className="flex justify-between mb-6">
              {otp.map((value, index) => (
                <motion.input
                  ...
                  onChange={(e) => handleChange(e, index)}
                  onKeyDown={(e) => handleKeyDown(e, index)}
                  className="w-12 h-12 text-center text-lg rounded-xl border border-white/20 dark:border-slate-600 bg-white/10 dark:bg-slate-800/50 text-white focus:border-amber-400 focus:shadow-[0_0_12px_rgba(255,179,71,0.5)] outline-none transition"
                  whileFocus={{ scale: 1.1 }}
                />
              ))}
            </div>
            ...
            {/* ✅ Timer & Resend */}
            <div className="flex justify-between items-center mb-5 text-gray-300 dark:text-slate-400 text-sm">
              <span>
                OTP expires in: {Math.floor(timeLeft / 60)}:
                {(timeLeft % 60).toString().padStart(2, "0")}
              </span>
              <button
                ...
                className={`ml-4 font-semibold ${
                  canResend
                    ? "text-amber-400 hover:text-orange-400 dark:text-amber-500 dark:hover:text-amber-300"
                    : "text-gray-500 dark:text-slate-600 cursor-not-allowed"
                }`}
              >
                Resend OTP
              </button>
          ...
          <motion.p
            whileHover={{ scale: 1.05 }}
            onClick={() => navigate("/")}
            className="mt-6 cursor-pointer text-center text-amber-400 dark:text-amber-500 hover:underline"
          >
            Back to Login
          </motion.p>
        </motion.div>
