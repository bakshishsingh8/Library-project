    ...
    <div className="relative space-y-2" ref={dropdownRef}>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className={`font-medium ${value ? "text-gray-900 dark:text-white" : "text-gray-500 dark:text-slate-400"}`}>
            {value || "Select category"}
          </span>
        </div>
          ...
          >
            {catList.map((cat) => (
              <li
                ...
                className={`px-4 py-3 cursor-pointer hover:bg-indigo-50 dark:hover:bg-slate-700 transition-all
                            font-medium text-gray-700 dark:text-slate-200 ${value === cat.title ? "bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300" : ""
                  }`}
              >
                {cat.title}
              </li>
        ...
        {open && (
          ...
          >
            <style>{`
              ...
              .dark .rdp-day_selected { background-color: #6366f1 !important; color: white; }
              .dark .rdp-day { color: #cbd5e1; }
              .dark .rdp-nav_button { color: #94a3b8; }
            `}</style>
            <DayPicker
              mode="single"
              selected={value ? new Date(value) : undefined}
              ...
              className="rounded-xl dark:text-slate-200"
              modifiersClassNames={{
                selected: "bg-indigo-600 text-white rounded-full",
                today: "text-indigo-600 font-bold underline",
              }}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <motion.div
        ...
        exit={{ y: 10, opacity: 0, scale: 0.98 }}
        transition={{ duration: 0.18 }}
        className="relative z-60 w-full max-w-2xl bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-100 dark:from-slate-800 dark:via-slate-900 dark:to-slate-800 border border-amber-200 dark:border-slate-700 shadow-2xl rounded-3xl p-6 md:p-8 mx-4 transition-colors duration-500"
      >
        <div className="flex items-center justify-between mb-6">
          <div className="flex flex-col">
            <h3 className="text-2xl text-amber-950 dark:text-white font-bold tracking-tight">
              Issue Book
            </h3>
        ...
        {success && (
          <div className="mb-5 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 px-4 py-3 rounded-xl flex items-center gap-2">
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
            <span className="font-medium">Book issued successfully!</span>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input
              ...
              value={form.studentName}
              onChange={(e) => handleChange("studentName", e.target.value)}
              className="w-full rounded-xl border border-stone-200 dark:border-slate-600 p-2.5 bg-white/80 dark:bg-slate-700 dark:text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 dark:focus:ring-amber-500/40 focus:border-amber-500 transition-all placeholder:dark:text-slate-400"
              placeholder="Enter student name..."
            />
            {errors.studentName && (
              <p className="text-red-500 text-xs font-medium mt-1 ml-1">{errors.studentName}</p>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-amber-200/50 dark:border-slate-700 mt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2.5 rounded-xl cursor-pointer text-stone-600 dark:text-slate-400 hover:bg-stone-100 dark:hover:bg-slate-700 font-medium  text-sm hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200"
            >
              Cancel
            </button>
            ...
            <button
              type="button"
              onClick={() => navigate("/issue-book-form", { state: { prefillData: form } })}
              className="px-5 py-2.5 cursor-pointer rounded-xl bg-white dark:bg-slate-800 border border-amber-300 dark:border-slate-600 text-amber-900 dark:text-amber-400 shadow-sm hover:bg-amber-50 dark:hover:bg-slate-700 font-medium  text-sm  hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200"
            >
              View Details
            </button>
            ...
            <button
              type="submit"
              className="px-6 py-2.5 rounded-xl bg-gray-900 dark:bg-white cursor-pointer text-white dark:text-slate-900 shadow-lg hover:bg-black dark:hover:bg-slate-100 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200 font-medium text-sm flex items-center gap-2"
            >
              <span>Confirm Issue</span>
              <svg className="w-4 h-4 text-gray-400 dark:text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
            </button>
          </div>
        </form>
      </motion.div>
...
function BooksList() {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      transition={{ duration: 0.5 }}
      className="bg-gradient-to-br from-amber-50 via-orange-100 to-amber-200 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 min-h-screen px-6 md:px-12 lg:px-20 md:mt-2 py-2 mt-[10px] pb-23 md:pb-15 transition-colors duration-500"
    >
      <Header
        viewMode={viewMode}
        setViewMode={setViewMode}
      ...
      <motion.h1
        ...
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 0.2 }}
        className="text-4xl mb-10 font-bold text-gray-700 dark:text-slate-200 text-center md:text-left"
      >
        {categoryName}
      </motion.h1>
      ...
      <AnimatePresence>
        {filteredBooks.length > 0 ? (
                ...
                {filteredBooks.map((item) => {
                  ...
                  return (
                    <motion.div
                      ...
                      exit="exit"
                      whileHover={{ scale: 1.03, y: -5, transition: { type: "spring", stiffness: 300 } }}
                      className={`bg-white dark:bg-slate-800 rounded-2xl shadow-md border overflow-hidden cursor-pointer mt-2 transition-colors duration-300
                         ${isHighlighted ? "border-amber-500 ring-4 ring-amber-200 dark:ring-amber-500/20" : "border-amber-100 dark:border-slate-700"}`}
                    >
                      <img src={item.img} alt={item.title} className="w-full h-44 sm:h-52 md:h-56 object-cover rounded-t-2xl" />
                      <div className={`p-4 flex flex-col justify-between ${isHighlighted ? "bg-amber-50 dark:bg-amber-900/10" : ""}`}>
                        <h2 className="text-base sm:text-lg h-20 font-bold text-gray-800 dark:text-slate-100">{item.title}</h2>
                        <p className="text-gray-600 dark:text-slate-400 text-sm mt-2 h-15 overflow-y-auto pr-2">{item.desc}</p>
                        <p className="text- sm:text-sm font-bold mt-3 text-gray-800 dark:text-slate-300">Rent Charges - ₹ 3/day </p>

                        <div className="mt-3 flex items-center justify-between">
                          <p className="text-amber-700 dark:text-amber-500 font-semibold text-lg">₹ {item.price}</p>
                ...
                {filteredBooks.map((item) => {
                  ...
                  return (
                    <motion.div
                      ...
                      exit="exit"
                      whileHover={{ scale: 1.01, x: 5, transition: { type: "spring", stiffness: 300 } }}
                      className={`flex flex-col sm:flex-row items-center p-4 sm:p-5 rounded-2xl shadow-md hover:shadow-lg border cursor-pointer overflow-hidden transition-colors duration-300
                         ${isHighlighted ? "bg-amber-50 dark:bg-amber-900/10 border-amber-500 ring-2 ring-amber-200 dark:ring-amber-500/20" : "bg-white dark:bg-slate-800 border-amber-100 dark:border-slate-700"}`}
                    >
                      <img src={item.img} alt={item.title} className="w-full sm:w-32 h-20 object-cover rounded-xl" />
                      <div className="sm:ml-5 mt-3 sm:mt-0 flex-1 text-center sm:text-left">
                        <h2 className="text-lg sm:text-xl font-bold text-gray-800 dark:text-slate-100">{item.title}</h2>
                        <p className="text-gray-600 dark:text-slate-400 text-sm mt-1 line-clamp-2">{item.desc}</p>
                        <p className="text-amber-700 dark:text-amber-500 font-semibold text-lg mt-2">₹ {item.price}</p>
                        <p className="text- sm:text-sm font-bold text-gray-800 dark:text-slate-300">Rent Charges - ₹ 3/day </p>
          ...
          <motion.p layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-gray-500 dark:text-slate-400 text-center mt-10">No books found.</motion.p>
        )}
      </AnimatePresence>
